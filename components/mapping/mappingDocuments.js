...
export function MappingDocuments({fileUploadName, hideDocumentManager = false, documentTypeId = null, companyId = null, locationId = null, userId = null, onManageField = null, onArchive = null}) {
    ...
    const documentColumns = [
        ...
        { field: 'documentTypeName', flex:1, headerName: 'Document Type', width: 150, editable: false },
        {
            ...
            getActions: ({ id, row }) => {
              return [
                <Button key={"manage-" + row.id} variant="text" startIcon={<ListRoundedIcon/>} onClick={() => {
                ...
                <Button key={"download-" + row.id} variant="text" startIcon={<DownloadRoundedIcon/>} onClick={() => {
                ...
                <Button key={"remove-" + row.id} variant="text" startIcon={<DeleteRoundedIcon/>} onClick={async () => {
                                ...
                                <FormControl fullWidth>
                                    ...
                                    <Select
                                        ...
                                        {componentTypes != null && componentTypes.map((componentType) => {
                                            return (<MenuItem key={componentType.id} value={componentType.id}>{componentType.name}</MenuItem>)
                                        })}
